---
name: to-tickets
description: Break a plan, spec, or the current conversation into a set of tracer-bullet tickets, each declaring its blocking edges, published to the local issue tracker.
...
Work from whatever is already in the conversation context. If the user passes a reference to a spec path as an argument, fetch it and read its full body and comments.
...
Do NOT close or modify any parent issue. The status can be either `ready`, `in-progress`, or `done`. The status must be set to `ready` for all tickets, so the agent can claim them in order. The agent will need to update the status to `in-progress` when it starts work, and `done` when it finishes. If it cannot finish, update back to `ready` so another agent can claim it.
